...
#+begin_src emacs-lisp
  (use-package isearch
    :ensure nil ; built-in
    :custom
    (isearch-lazy-count t)    ; show number of hits in modeline
    (isearch-allow-scroll t)) ; allow scrolling during search
#+end_src
** spellchecking
Load appropriate personal dictionary based on ~ispell-local-dictionary~ variable.
#+begin_src emacs-lisp
    (use-package ispell
      :ensure nil ; built-in
