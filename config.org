...
#+end_src
*** Disable opening a new buffer per folder
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp :tangle no
  (defun yr/bibtool-current-file ()
    "Run bibtool on the current buffer's file, using a local or global .bibtoolrsc configuration."
    (interactive)
    (let ((file (buffer-file-name)))
      (if file
          (let ((default-directory (file-name-directory file))
                (base-file (file-name-nondirectory file)))
            (if (file-exists-p ".bibtoolrsc")
                (shell-command (concat "bibtool " base-file " -o " base-file))
              (shell-command (concat "bibtool -r ~/.bibtoolsrc " base-file " -o " base-file))))
